...
(5, 'CAN_EDIT_PERMISSIONS', 'User can modify access permissions of a squadron member', 5, 0),
...
INSERT INTO rank (id, group_code, assigned_id, `name`, perm_mask) VALUES (1, 'squadron', 0, 'Rookie', 0),
                                                                         (2, 'squadron', 1, 'Agent', 0),
                                                                         (3, 'squadron', 2, 'Officer', 0),
                                                                         (4, 'squadron', 3, 'Senior Officer', 0),
                                                                         (5, 'squadron', 4, 'Leader', 0),
...
CREATE TABLE x_leaderboard_report (
  ...
  sort_columns varchar(512) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
--
-- Dumping data for table x_player_report
--

INSERT INTO x_player_report (id, title, header, `columns`, `sql`, count_sql, parameters, parameters_sql, order_id, order_dir, cast_columns, sort_columns) VALUES
(1, 'Earning History (Complete)', '[\"Date\",\"Type\",\"Amount\",\"Crew Paid\"]', '[\"earned_on\",\"earning_type\",\"reward\",\"crew_wage\"]', 'select u.commander_name, eh.user_id, eh.squadron_id, eh.earned_on, if(eh.earning_type_id=\'8\' and eh.notes is not null,concat(et.name, \' (\', eh.notes, \')\') ,et.name) as earning_type, format(eh.reward,0) as reward, eh.reward as sort_reward, format(eh.crew_wage,0) as crew_wage, eh.crew_wage as sort_crew_wage from earning_history eh right outer join user u on eh.user_id = u.id left join earning_type et on eh.earning_type_id = et.id where eh.user_id=?', 'select count(*) from earning_history where user_id=?', '[[\"id\"],[\"id\"]]', 'select * from user where id=?', 0, 'desc', NULL, '{\"reward\":\"sort_reward\",\"crew_wage\":\"sort_crew_wage\"}'),
(2, 'Earning History Summary', '[\"Type\",\"Count\",\"Total Earned\",\"Total Crew Paid\"]', '[\"earning_type\",\"num_transactions\",\"reward\",\"crew_wage\"]', 'select u.commander_name, eh.user_id, eh.squadron_id, et.name as earning_type, count(eh.id) as num_transactions, format(sum(eh.reward),0) as reward, sum(eh.reward) as sort_reward, format(sum(eh.crew_wage),0) as crew_wage, sum(eh.crew_wage) as sort_crew_wage from earning_history eh right outer join user u on eh.user_id = u.id left join earning_type et on eh.earning_type_id = et.id where eh.user_id=? group by earning_type_id', 'select count(*) from (select id from earning_history where user_id=? group by earning_type_id) a', '[[\"id\"],[\"id\"]]', 'select * from user where id=?', 0, 'desc', NULL, '{\"reward\":\"sort_reward\",\"crew_wage\":\"sort_crew_wage\"}');
...
--
-- Indexes for table x_player_report
--
ALTER TABLE x_player_report
  ADD PRIMARY KEY (id);
...
--
-- AUTO_INCREMENT for table x_player_report
--
ALTER TABLE x_player_report
